 ...
 * importScripts関数の場合は読み込むファイルのパスを
 * Wokerファイルからの相対パスで指定する必要があるが
 * このモジュールではimportするファイルを
 * import元ファイルからの相対パスで指定できるのが利点.
 *
 * usage:
 *   ・クライアントページ
...
// 実行環境名の取得
$.environment = (function(globalObject) {
 ...
 *     宣言する名前空間名. ドットで繋げることでサブ名前空間の宣言も可
...
/**
 * originを基準とした相対パスを返す
 * originとtargetが同一の場合は空文字列を返す
 * targetとoriginのディレクトリを指している場合はドット"."を返す
 * 
...
ScriptLoader.prototype.import = function(path) {
  if (this.phase > ScriptLoader.PHASE_LOADING) {
    throw new Error("Script Loader has already executed.");
  }
...
/**
 * リソースの登録
 * @param resourceName リソースの登録名
 * @param type
 *   リソースの型. 以下から選択
 *   "text": テキストファイルとして読み込み、文字列で保存
 *   "json": JSON形式のテキストファイルとして読み込み、オブジェクトで保存
 *   "binary": バイナリファイルとして読み込み、ArrayBufferで保存
 */
ScriptLoader.prototype.resource = function(name, path, type) {
...
ScriptLoader.prototype.execute = function() {
  ...
  if (this.debugMode) {
    ...
    this.spentTime = Date.now() - this.spentTime;
  }
...
$.getSpentTime = function() {
  return _scriptLoader.spentTime;
};
 ...
 * @param {Function} code
 * 
 * code(String environment, Function code)
 * @param {String} environment コード定義の対象とする環境
 * WINDOW,WORKER
 * 
 */
$.code = function(arg1, arg2) {
